...
class QuestionData(BaseModel):
    ...
    explanation: str = Field(
        description=
        "Explanation of why the correct answer is right and others are wrong")
    comment: str = Field(
...
def render_chat_interface():
    ...
    # Custom CSS for fixed control bar, full-width design, and message alignment
    st.markdown("""
    <style>
    /* Remove white background from app */
    .stApp {
    ...
    /* Chat message alignment - user right, agent left */
    ...
    # Control bar with only the view selector
    ...
    if has_unsaved_feedback():
        st.info(
            "💡 You have unsaved feedback. Save it before continuing or it will be lost."
        )
    ...
    chat_container = st.container(height=None, border=False)
    with chat_container:
        ...
        for i, (sender, message) in enumerate(st.session_state.chat_history):
            if sender == "You":
                ...
                with st.chat_message("assistant"):
                    st.markdown(message)
                    # Add feedback buttons inline for AcadGenie messages
                    render_inline_feedback_buttons(i)
        
        st.markdown('</div>', unsafe_allow_html=True)

    # Fixed input container at bottom - moved outside chat container
    st.markdown('<div class="input-container">', unsafe_allow_html=True)
    
    # Input row with chat input (80%) and action buttons (20%)
    input_col, buttons_col = st.columns([8, 2])

    with input_col:
        if not user_input:  # Only show input if not handling pending question
            user_input = st.chat_input("Type your message here...",
                                       key="main_chat_input")

    with buttons_col:
        # Create three small columns for the buttons
        btn_col1, btn_col2, btn_col3 = st.columns(3)

        with btn_col1:
            if st.button("💾",
                         key="save_btn_input",
                         help="Save chat to database",
                         type="primary"):
                print(
                    "==============================Feedback===========================\n\n",
    ...
    if user_input:
        ...
        with st.spinner("AcadGenie is thinking..."):
            try:
                ...
                ai_response = result.get("acadgenie",
